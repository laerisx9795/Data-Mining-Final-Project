...
def renyi_entropy(probabilities, alpha):
    # If alpha = 1, then the computation is same as Shannon Entropy.
    if alpha == 1:
...
### Tsallis Entropy ###
q = 2
def tsallis_entropy(probabilities, q):
    # If q = 1, then the computation is same as Shannon Entropy.
    if q == 1:
        return -np.sum(probabilities * np.log2(probabilities))
    else:
...
def compute_feature_bias(x, uncertain_features, entropy_type, sample_indices=None):
...
###############################################################################
                    if entropy_type == 0:
                        ...
                        entropy_sum += renyi_entropy(np.array([v, 1.0 - v]), alpha)
                    elif entropy_type == 2:
                        entropy_sum += tsallis_entropy(np.array([v, 1.0 - v]), q)
###############################################################################
                else:
                    missing_values += 1
...
class UDBaggingClassifier(BaggingClassifier):
    ...
    def _fit(self, X, y, max_samples=None, max_depth=None, sample_weight=None):
        ...
        if self.biased_subspaces:
            feature_bias = compute_feature_bias(X, self.uncertain_features, self.entropy_type)
